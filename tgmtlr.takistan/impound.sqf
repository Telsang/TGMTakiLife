...
private ["_crew","_incarpark","_moneh","_vcl","_art"];
liafu = true;
_vcl = _this select 0;
_art = _this select 1;
...
if(_art == "impound")then {
		...
		if(!alive _vcl)exitwith{player groupchat "Removing Wreck"; deleteVehicle _vcl; };
...
if(_art == "buy")then {
		...
		["money", -impoundpay] call INV_AddInventoryItem;
