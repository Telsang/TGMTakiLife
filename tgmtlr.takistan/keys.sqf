...
private ["_index","_item","_anzahl","_vcl","_sel","_spielernum","_spieler","_playerobject","_closeplayers","_art"];
_art = _this select 0;
if (_art == "use") then
 {
...
_spieler = INV_PLAYERLIST select _spielernum;
...
player groupChat localize "STRS_inv_inventar_uebergabe_self";
   ...
   if (!(_playerobject in _closeplayers) and (player distance _playerobject > 20))  exitWith {
player groupChat localize "STRS_inv_inventar_uebergabe_distance";
};

if ((!alive _vcl) or (isNull _vcl)) exitWith {};
