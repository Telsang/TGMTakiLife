...
private ["_civkopfgeld","_gesucht","_killer","_killedbyvehicle","_self"];
...
if (_killer in civarray) then {
	...
	["CivsKilled", CivsKilled] call ClientSaveVar;
	...
	if (iscop) then {
		...
		if (_civkopfgeld != 0) then {
			(format['if (player == %3)then{[%2] call bank_transaction; player groupChat "You got 1/3 of the civs bounty totaling $%2"}', player, _civkopfgeld, _self]) call broadcast;
		};
...
if (_killer in coparray) then {
	...
	["CopsKilled", CopsKilled] call ClientSaveVar;
